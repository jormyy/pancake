...
**The probe can now measure: its parser and early observer both work on real Chromium. What does not hold is the record of the repair. Of the 26 rows in iteration 19, two "Fixed" rows are false (round-6 items 18 and 24), and five are only partly true (items 3, 7, 8, 22 and 23). The row for item 2 also overclaims its tests (findings 1, 2, 3, 5, 8 and 11 below).**
...
| 12 — stale header | **Fixed.** The screenshot sentence is a separate nit (item 26) |
| 13 — mid-life override not in soak records | **Recorded.** Attempt 2's report proves its flag. No retained file shows attempt 1's flag (item 12) |
...
### 2. Three WORKLOG rows say "Fixed" for fixes that did not fully land · minor · pre-merge ask
...
- **Fix:** make the three edits, or change the rows to "Not fixed". Also correct the rows for items 2, 7, 8, 22 and 24, as findings 3, 5, 8, 11 and 1 describe.
...
- **Why not gating:** the soak fails loud. With no state file, `soak.mjs:98-99` throws (the release script sets `E2E_ENABLE_BACKEND_TICKS=1`). With a stale one, `soak-support.mjs:521` throws before season 1. It cannot pass by mistake.
...
`tests/e2e/pwa-paint-probe.mjs:357-364`.
...
27. **A post-open socket drop stalls `finish()` for 10 s** · nit · pre-existing. `openCdpClient` (`browser-agent.mjs:69-123`) has no listener for the socket's own `close` event, so nothing rejects pending sends. The stall is after the read, so it never changes a measurement.

- Carried unchanged: round-5 items 1–10, round-4 items 10 and 11, and round-3 items 10, 18, 20, 21, 22, 30 and 33.

**Top asks: items 1 and 2, because the record says they are fixed. Items 3–8 are the other pre-merge asks. Item 9 is the lead to follow before the next soak. Everything else is nit. Nothing gates.**
...
<sub>Evidence is `file:line` at HEAD `24d4768` vs `c8945c4`. Ledger: 36 candidates, 9 refuted, 27 survived, every one terminal. An independent judge checked about 70 citations. It scored grounding 5, calibration 5, scope honesty 4 and actionability 5, and it raised no blocker or material finding against the review. Its corrections are applied.</sub>
