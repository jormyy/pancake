...
app.post('/draft/:draftId/nominate', async (req: any, reply) => {
    try {
        ...
        if (!memberId || !playerId) {
            reply.status(400)
            return { ok: false, error: 'memberId and playerId required' }
        }
...
app.post('/draft/:draftId/bid', async (req: any, reply) => {
    try {
        ...
        if (!memberId || !nominationId || amount == null) {
            reply.status(400)
            return { ok: false, error: 'memberId, nominationId, and amount required' }
        }
