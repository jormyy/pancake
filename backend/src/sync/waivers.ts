 ...
 *
 * The database RPC processes one claim per transaction, using current
 * waiver_priorities and league-season row locks so priority changes made by a
 * successful claim affect the next claim in the same run.
 */
export async function processWaiverClaims(): Promise<void> {
    ...
    if (processed === 0) {
        ...
        console.log(`[waivers] Processed ${processed} claim(s).`)
    }
}
