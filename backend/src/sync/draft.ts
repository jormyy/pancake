...
export async function startDraft(leagueId: string) {
  ...
  console.log(`[draft] Started draft ${draft.id} for league ${leagueId} with ${members.length} managers`)
  return draft
}
...
export async function placeBid(
  ...
  if (amount <= nom.current_bid_amount) throw new Error(`Bid must exceed current bid of $${nom.current_bid_amount}`)
  ...
  const { data: budget } = await supabase
    ...
    .eq('member_id', memberId)
    .single()
  if (!budget || budget.remaining < amount) throw new Error(`Insufficient budget (you have $${budget?.remaining ?? 0} remaining)`)
