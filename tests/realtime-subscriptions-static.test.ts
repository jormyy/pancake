...
describe('shared realtime subscription coverage', () => {
    ...
    it('wires shared app surfaces to realtime refreshes with focus refresh fallback intact', () => {
        ...
        expect(leagueScreenState).toContain("table: 'roster_transactions'")
        expect(leagueScreenState).toContain("table: 'draft_picks'")
        expect(leagueScreenState).toContain("filter: `league_id=eq.${lid}`")
        expect(leagueScreenState).toContain('currentDraftIds.has(draftId)')
        ...
        expect(tradesScreen).toContain("table: 'trades'")
        expect(tradesScreen).toContain("table: 'trade_items'")
        expect(tradesScreen).toContain("table: 'trade_participants'")
        expect(tradesScreen).toContain("table: 'trade_vetos'")
        expect(tradesScreen).toContain("table: 'trade_block_items'")
        expect(tradesScreen).toContain("filter: `league_id=eq.${leagueId}`")
