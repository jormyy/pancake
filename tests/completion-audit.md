...
| Production blocker check | BLOCKED | `npm run prod:check` writes `tests/production-readiness-report.md`. Current report verifies Supabase CLI/link/API-key metadata, hosted Edge `SUPABASE_SECRET_KEYS`, modern Supabase API-key records, local frontend `sb_publishable_` usage, local backend `sb_secret_` usage, and hosted Fastify `/health`; it still blocks on missing `SUPABASE_DB_PASSWORD`, linked migration dry-run timeout, unverified hosted Fastify secret-key env, and unverified remote legacy JWT disable/revocation. This branch adds `/health.supabaseAdminKeyMode` so a deployed backend can prove `modern-secret` without exposing secrets, but the current hosted deployment does not yet expose that field. |
