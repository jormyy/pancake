...
Browser multi-team trade proposal gameplay is available through `npm run e2e:browser-trade-multi-team`. It creates an isolated three-user league, opens the real multi-team composer through `agent-browser`, selects each team column, changes one destination route, selects one player from each team, submits through the authenticated Edge API route, and verifies one pending multi-team trade with three participants and routed `trade_items`.
