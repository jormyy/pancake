...
Mid-life migration checks are available with `E2E_ENABLE_MIDLIFE_MIGRATION=1` or `--midlife-migration=true`. The release workflow starts from the schema immediately before `20260709100026_index_pending_trade_feed.sql`, then the runner applies repository head between seasons 5 and 6. `E2E_MIDLIFE_EXPECTED_VERSION` and `SUPABASE_DB_URL` are required so the runner can compare the migration ledger before and after, require a nonempty strictly advancing delta, and reject an already-current database. Override the boundary with `E2E_MIDLIFE_MIGRATION_AFTER_SEASON`. Artifacts are written to `tests/artifacts/season-<N>/midlife-migration.json`.
