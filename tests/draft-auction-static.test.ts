...
describe('logic hardening source guards - draft, auction, roster history', () => {
    ...
    it('keeps rookie pick clocks server-authoritative through SQL and Edge cron', () => {
        ...
        expect(rookieRoom).toContain('useDraftAdminControls')
        expect(read('components/league/draft-room/useDraftAdminControls.ts')).toContain('await refresh()')
    ...
    it('keeps rookie timeout behavior configurable and server-enforced', () => {
        ...
        expect(draftSetupPanels).toContain('Timeout behavior')
        expect(draftChips).toContain('ROOKIE_TIMER_EXPIRY_BEHAVIORS')
    ...
    it('supports configurable real drafts and side-effect-free mock drafts', () => {
        ...
        const leagueState = read('hooks/use-league-screen-state.ts')
        ...
        expect(leagueState).not.toContain("includeCompletedRookie: currentLeague.status === 'drafting'")
        expect(leagueState).toContain('createMockDraftRoom')
        expect(leagueTabs).toContain('Mock Rooms')
        expect(mockRoomsPanel).toContain('Create Room')
        expect(leagueState).toContain('OPEN_DRAFT_STATUSES')
        expect(leagueState).toContain('getJoinableDraft')
        expect(draftActiveState).toContain('Resolve Rookie Draft')
        expect(leagueState).toContain('includeCompletedRookie: true')
        expect(leagueState).toContain('const draft = activeDraft ?? await getJoinableDraft(currentLeague.id')
        expect(draftActiveState).toContain('activeDraftButtonLabel')
        expect(draftActiveState).toContain("draft.isMock ? 'Mock ' : ''")
        expect(draftActiveState).toContain('activeDraft.draftType === \'snake\' && !activeDraft.isMock && isCommissioner')
