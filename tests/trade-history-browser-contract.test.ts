...
describe('trade history browser contract', () => {
    it('uses a participant-owned, server-filtered history resource', () => {
        ...
        expect(historyQuery).toContain('.limit(pageSize + 1)')
        expect(historyQuery).toContain('const hasMore = refs.length > pageSize')
        expect(historyQuery).toContain('const pageRefs = refs.slice(0, pageSize)')
