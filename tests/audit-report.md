...
- D.X.5 browser route sweep added behind `E2E_BROWSER_FULL_SWEEP=1`: the agent-browser smoke now visits auth, tab, modal, and player routes with seeded Supabase IDs, writes screenshots and console/error artifacts, and fails on uncaught browser errors. `E2E_BROWSER_FULL_SWEEP=1 npm run e2e:browser-smoke` passed against local Expo web on port 8081, visiting sign-in, sign-up, Home, Players, Roster, Trades, League, create/join league, commissioner settings, lineup, bracket, claim-player, player detail, propose-trade, and team-roster. Auction and rookie draft rooms were skipped because the seeded league had no draft rows; full gameplay remains pending.
