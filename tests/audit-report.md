...
- D.LONG.3/D.LONG.4 history-retention slice added behind `E2E_ENABLE_HISTORY=1`: the harness can seed deterministic completed-season standings plus finalized champion rows and verify they remain queryable after real backend season resets. Verification is currently blocked before the assertion runs because backend-tick mode still stops at the approval-gated `/e2e/process-waivers` 500 from `process_next_waiver_claim_atomic(...)` (`42702`, ambiguous `player_id`).
- D.X.2 matchups realtime slice added behind `E2E_ENABLE_REALTIME=1`: the harness opens 10 seeded-user Supabase Realtime clients by default, subscribes each to a disposable `matchups` row, updates that row through the service-role client, and requires every client to receive the update within 2s. This covers the live-score/matchups-update channel only; auction bid-room realtime still needs a draft-room scenario. Verification currently fails before delivery measurement because Realtime subscription returns `TIMED_OUT` for client 1. Do not silently change realtime configuration or weaken the assertion; approval is needed before fixing this newly exposed D.X.2 failure.
