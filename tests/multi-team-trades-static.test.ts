...
describe('multi-team trade UI and client mapping', () => {
    ...
    it('exposes a multi-team composer and participant-aware trade card actions', () => {
        ...
        expect(multiTeamComposer).toContain('const currentIndex = ids.indexOf(memberId)')
        expect(multiTeamComposer).toContain('ids[(currentIndex + 1) % ids.length]')
        ...
        expect(tradeCard).toContain('routedTradeGroups(trade, myMemberId)')
        expect(tradeCard).toContain('group.fromLabel} sends to {group.toLabel')
        expect(tradeCard).toContain('trade.routedItems')
        expect(tradeCard).toContain('item.fromMemberId')
