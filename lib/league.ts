...
export async function createLeague(
  ...
  const { data: league, error: leagueError } = await supabase
    .from('leagues')
    .insert({ name, slug, invite_code: inviteCode, commissioner_id: userId, auction_budget: auctionBudget })
    .select()
    .single()
  ...
  const { error: memberError } = await supabase
    .from('league_members')
    .insert({ league_id: league.id, user_id: userId, role: 'commissioner', team_name: teamName })
