...
export async function getMyMatchup(
  ...
  const { data, error } = await supabase
    .from('matchups')
    .select('id, week_number, home_points, away_points, winner_member_id, is_finalized, home_member_id, away_member_id')
    .eq('league_id', leagueId)
...
export async function getLeagueStandings(leagueId: string): Promise<StandingRow[]> {
  ...
  for (const m of members ?? []) {
    map[m.id] = { memberId: m.id, teamName: m.team_name ?? 'Team', wins: 0, losses: 0, pointsFor: 0, pointsAgainst: 0 }
  }
