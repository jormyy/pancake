...
export async function getDraftState(draftId: string): Promise<DraftState | null> {
  ...
  const mappedNominations: Nomination[] = (nominations ?? []).map((n: any) => ({
    ...
    player: n.players
      ? { displayName: n.players.display_name, nbaTeam: n.players.nba_team, position: n.players.position }
      : null,
  }))
...
export function subscribeToDraft(draftId: string, onChange: () => void): RealtimeChannel {
  const channel = supabase
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'nominations', filter: `draft_id=eq.${draftId}` },
      onChange,
    )
    .on(
      'postgres_changes',
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'draft_budgets', filter: `draft_id=eq.${draftId}` },
      onChange,
    )
    .on(
      'postgres_changes',
