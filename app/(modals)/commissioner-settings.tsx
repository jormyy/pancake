        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            {SCORING_FIELDS.map(({ key, label }, i) => (
              <View key={key} style={[styles.row, i < SCORING_FIELDS.length - 1 && styles.rowBorder]}>
                <Text style={styles.rowLabel}>{label}</Text>
                <TextInput
                  style={styles.scoreInput}
                  value={scoring[key] ?? ''}
                  onChangeText={(v) => setScoring((prev) => ({ ...prev, [key]: v }))}
          ...
          <View style={styles.card}>
            {SLOT_TYPES.map((type, i) => (
              <View key={type} style={[styles.row, i < SLOT_TYPES.length - 1 && styles.rowBorder]}>
                <Text style={styles.rowLabel}>{type}</Text>
                <View style={styles.stepper}>
                  <TouchableOpacity style={styles.stepBtn} onPress={() => adjustSlot(type, -1)}>
          ...
          <View style={styles.card}>
            {[
              ...
              { label: 'Auction Budget ($)', value: auctionBudget, set: setAuctionBudget },
              { label: 'Playoff Start Week (18–22)', value: playoffWeek, set: setPlayoffWeek },
...
const styles = StyleSheet.create({
  ...
  scoreInput: {
    width: 72, textAlign: 'right', fontSize: 15, fontWeight: '600',
    color: '#F97316', padding: 0,
  },

  stepper: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  stepBtn: {
    width: 30, height: 30, borderRadius: 8, backgroundColor: '#f3f3f3',
    justifyContent: 'center', alignItems: 'center',
  },
  stepBtnText: { fontSize: 18, color: '#333', lineHeight: 22 },
  stepValue: { fontSize: 16, fontWeight: '700', color: '#111', minWidth: 20, textAlign: 'center' },

  saveButton: {
    marginTop: 16, backgroundColor: '#F97316', borderRadius: 14,
