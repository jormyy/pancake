    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.rosterCount}>
          {active.length}/{league?.roster_size ?? 20} active · {ir.length}/{league?.ir_slots ?? 2} IR
        </Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            if (item._isHeader) {
              return <View style={styles.sectionHeader}><Text style={styles.sectionHeaderText}>IR</Text></View>
            }
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <View style={[styles.avatar, { backgroundColor: POSITION_COLORS[pos] ?? '#ccc' }]}>
                  <Text style={styles.avatarText}>
                    {player.display_name.split(' ').map((w: string) => w[0]).slice(0, 2).join('')}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  injuryBadge: { alignSelf: 'flex-start', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, marginTop: 2 },
  injuryText: { color: '#fff', fontSize: 10, fontWeight: '700' },

  irButton: {
    paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8,
    borderWidth: 1, borderColor: '#ddd', minWidth: 52, alignItems: 'center',
  },
