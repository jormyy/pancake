...
bounded_players AS (
  SELECT
    ...
    contend_ranking.source_rank AS contend_rank,
    rebuild_ranking.source_rank AS rebuild_rank,
    rookie_ranking.source_rank AS rookie_rank,
    ranking.rank_change,
    ranking.age,
  ...
  LEFT JOIN LATERAL (
    SELECT row.source_rank
      FROM public.dynasty_rankings AS row
     WHERE row.player_id = player.id
       AND row.source = 'hashtagbasketball.com/contend'
     ORDER BY row.fetched_at DESC, row.source_rank ASC, row.id ASC
     LIMIT 1
  ) AS contend_ranking ON true
  LEFT JOIN LATERAL (
    SELECT row.source_rank
      FROM public.dynasty_rankings AS row
     WHERE row.player_id = player.id
       AND row.source = 'hashtagbasketball.com/rebuild'
     ORDER BY row.fetched_at DESC, row.source_rank ASC, row.id ASC
     LIMIT 1
  ) AS rebuild_ranking ON true
  LEFT JOIN LATERAL (
    SELECT row.source_rank
      FROM public.dynasty_rankings AS row
     WHERE row.player_id = player.id
       AND row.source = 'hashtagbasketball.com/rookie'
     ORDER BY row.fetched_at DESC, row.source_rank ASC, row.id ASC
     LIMIT 1
  ) AS rookie_ranking ON true
