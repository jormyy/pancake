-- Dynasty transactions/trade release: counters, edits, expiration, trade block, and FAAB in trades.

ALTER TABLE public.trades
  ADD COLUMN IF NOT EXISTS proposer_faab_amount int NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS recipient_faab_amount int NOT NULL DEFAULT 0;

ALTER TABLE public.trades
  DROP CONSTRAINT IF EXISTS trades_faab_amounts_valid;

ALTER TABLE public.trades
  ADD CONSTRAINT trades_faab_amounts_valid
    CHECK (proposer_faab_amount >= 0 AND recipient_faab_amount >= 0);
...
CREATE TRIGGER clear_trade_block_listing_on_inactive_roster
AFTER UPDATE OF is_on_ir, is_on_taxi ON public.roster_players
FOR EACH ROW
WHEN (
  (NEW.is_on_ir = true OR NEW.is_on_taxi = true)
  AND (
    OLD.is_on_ir IS DISTINCT FROM NEW.is_on_ir
    OR OLD.is_on_taxi IS DISTINCT FROM NEW.is_on_taxi
  )
)
EXECUTE FUNCTION private.clear_trade_block_listing_on_inactive_roster();
...
DROP FUNCTION IF EXISTS public.propose_trade_atomic(uuid, uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text);
...
DROP TRIGGER IF EXISTS prevent_expired_or_unfunded_trade_accept ON public.trades;
CREATE TRIGGER prevent_expired_or_unfunded_trade_accept
  BEFORE UPDATE OF status ON public.trades
  FOR EACH ROW
  EXECUTE FUNCTION private.prevent_expired_or_unfunded_trade_accept();
...
REVOKE ALL ON FUNCTION public.propose_trade_atomic(uuid, uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamptz, int, int) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.propose_trade_atomic(uuid, uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamptz, int, int) TO service_role;

REVOKE ALL ON FUNCTION public.counter_trade_atomic(uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamptz, int, int) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.counter_trade_atomic(uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamptz, int, int) TO service_role;

REVOKE ALL ON FUNCTION public.edit_trade_atomic(uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamptz, int, int) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.edit_trade_atomic(uuid, uuid, uuid, uuid[], uuid[], uuid[], uuid[], text, timestamptz, int, int) TO service_role;
...
REVOKE ALL ON FUNCTION public.complete_accepted_trade_atomic(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.complete_accepted_trade_atomic(uuid) TO service_role;

REVOKE ALL ON FUNCTION public.add_trade_block_item_atomic(uuid, uuid, uuid, uuid, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.add_trade_block_item_atomic(uuid, uuid, uuid, uuid, text, uuid) TO service_role;

REVOKE ALL ON FUNCTION public.remove_trade_block_item_atomic(uuid, uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.remove_trade_block_item_atomic(uuid, uuid, uuid) TO service_role;
