-- Score finalization hardening:
-- - Commit score reconciliation, matchup finalization, and standings snapshots
--   in a single database transaction. Notification delivery remains outside
--   the transaction and receives only rows that became finalized in this call.
...
REVOKE ALL ON FUNCTION public.finalize_score_week_atomic(uuid, uuid, int, jsonb, jsonb, timestamptz, timestamptz) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_score_week_atomic(uuid, uuid, int, jsonb, jsonb, timestamptz, timestamptz) TO service_role;
