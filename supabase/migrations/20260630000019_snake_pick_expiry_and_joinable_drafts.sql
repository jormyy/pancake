-- Prod replay for rookie draft expiry hardening after 20260630000018:
-- manual snake picks must not bypass an expired pick timer, while the server
-- auto-pick path can still resolve expired auto-pick drafts atomically.
...
BEGIN
  ...
  v_result := private.make_snake_pick_atomic_internal(
    ...
    p_member_id,
    v_player_id,
    COALESCE(v_draft.timer_expiry_behavior, 'auto_pick') = 'auto_pick'
  );
