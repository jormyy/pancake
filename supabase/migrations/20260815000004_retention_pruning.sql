-- Retention for unbounded-growth tables. Keeps everything the product reads:
-- current + previous season lineups, every season's final standings snapshot
-- (history/champion views), and three seasons of transaction history. Prunes
-- only ops telemetry and per-week rows no UI query returns.
...
REVOKE ALL ON FUNCTION public.prune_unbounded_history() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.prune_unbounded_history() FROM anon;
REVOKE ALL ON FUNCTION public.prune_unbounded_history() FROM authenticated;
GRANT EXECUTE ON FUNCTION public.prune_unbounded_history() TO service_role;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.schedule(
      'retention-prune',
      '0 10 * * 0',
      $job$SELECT public.prune_unbounded_history()$job$
    );
  END IF;
END $$;
