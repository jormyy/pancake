ALTER TABLE public.drafts
  ...
  ADD COLUMN IF NOT EXISTS pick_timer_seconds int NOT NULL DEFAULT 30,
  ADD COLUMN IF NOT EXISTS rounds int;

ALTER TABLE public.drafts
  DROP CONSTRAINT IF EXISTS drafts_pick_timer_seconds_range,
  DROP CONSTRAINT IF EXISTS drafts_rounds_positive;

ALTER TABLE public.drafts
  ADD CONSTRAINT drafts_pick_timer_seconds_range CHECK (pick_timer_seconds BETWEEN 5 AND 3600),
  ADD CONSTRAINT drafts_rounds_positive CHECK (rounds IS NULL OR rounds > 0);
...
CREATE UNIQUE INDEX drafts_one_open_per_season
  ...
  WHERE status IN ('pending', 'in_progress') AND is_mock = false;

DROP FUNCTION IF EXISTS public.start_auction_draft_atomic(uuid, text);
...
REVOKE ALL ON FUNCTION public.create_auction_nomination_atomic(uuid, uuid, uuid, uuid, int) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.place_auction_bid_atomic(uuid, uuid, uuid, int, uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.close_auction_nomination_atomic(uuid) FROM PUBLIC, anon, authenticated;
