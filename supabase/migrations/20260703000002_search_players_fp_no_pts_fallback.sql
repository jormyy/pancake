-- search_players surfaced avg_points as avg_fantasy_points whenever the
-- per-league fantasy average was missing, so the FP column exactly duplicated
-- PTS (visible to any points-league manager) and sorted by the wrong metric.
-- With v_player_avg_fantasy_points now covering uncached leagues
-- (20260703000001), the remaining honest behavior is: no league-scored average
-- -> NULL (renders as an em dash), never plain points dressed as FP.
