-- ──────────────────────────────────────────────────────────────────────────
-- advance_season_atomic: gate on leagues.status IN ('playoffs','archived')
-- ──────────────────────────────────────────────────────────────────────────
-- Problem (SLICE A, iter 36):
--   The SECURITY DEFINER RPC advance_season_atomic accepted any league
--   regardless of leagues.status. A commissioner could trigger
--   "Advance to Next Season" during 'setup' / 'drafting' / 'active' and
--   roll over rosters, regenerate draft_picks for the +5-year horizon,
--   reseed waiver priorities, and flip the league to 'offseason' while
--   the prior season's draft or regular-season matchups were still live.
--
-- Fix:
--   After locking and re-fetching the league row, raise a user-facing
--   P0001 error unless the status is one of ('playoffs','archived').
--   These are the only states where a season has materially concluded
--   (regular season + championship resolved, or league fully wound down)
--   and a rollover is meaningful. All other logic — current-season lookup,
--   year-collision check, league_seasons flip, roster carry-over,
--   draft_picks +5y generation, waiver_priorities reseed, status flip to
--   'offseason' — is preserved verbatim from 20260512000001.
--
-- Grants:
--   advance_season_atomic was already locked down to service_role only by
--   20260512000001 (REVOKE FROM PUBLIC/anon/authenticated, GRANT TO
--   service_role). Those grants are re-issued here so the migration is
...
BEGIN
  ...
  -- Status gate: rollover only meaningful once the season has concluded.
