...
BEGIN
  ...
  PERFORM 1
   ...
   WHERE id = p_league_id
     ...
     AND (trade_deadline IS NULL OR trade_deadline >= (now() AT TIME ZONE 'America/New_York')::date)
   FOR SHARE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Trades can only be proposed during the active season before the trade deadline.';
  END IF;
