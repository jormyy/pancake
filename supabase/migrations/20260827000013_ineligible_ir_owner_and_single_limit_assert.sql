-- One owner for the ineligible-IR list; the weekly limit asserted once per path.
--
-- add_free_agent_atomic and the waiver processor read
-- private.ineligible_ir_player_names instead of carrying their own copies; the
-- add raises it as SQLSTATE PA005 so a stale client can open the IR flow.
-- private.consume_weekly_add only counts: the limit is asserted once, before
-- side effects, by the paths that add. release_live_poll_lock follows its
-- dropped pair.


DROP FUNCTION IF EXISTS public.release_live_poll_lock();
...
BEGIN
  ...
  v_ineligible := private.ineligible_ir_player_names(v_claim.league_id, v_claim.league_season_id, v_claim.member_id);
  IF v_ineligible IS NOT NULL THEN
    v_failure := format(
