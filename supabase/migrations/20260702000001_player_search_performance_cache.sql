-- Player search hot path:
-- - Materialize league-scored season fantasy averages instead of aggregating
--   v_fantasy_points on every search_players call.
-- - Page the sorted player set before projection enrichment so one tab load
--   asks get_league_projection_rows for ~60 players, not the full pool.

CREATE SCHEMA IF NOT EXISTS analytics;

CREATE MATERIALIZED VIEW IF NOT EXISTS analytics.mv_player_avg_fantasy_points AS
SELECT
  fp.league_id,
  fp.player_id,
  fp.season_year,
...
INNER JOIN public.player_game_stats pgs
  ON pgs.id = fp.stat_id
 AND NOT pgs.did_not_play
GROUP BY fp.league_id, fp.player_id, fp.season_year;

CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_player_avg_fantasy_points_unique
  ON analytics.mv_player_avg_fantasy_points(league_id, player_id, season_year);

CREATE INDEX IF NOT EXISTS idx_mv_player_avg_fantasy_points_sort
...
GRANT SELECT ON analytics.mv_player_season_averages TO authenticated, anon, service_role;
GRANT SELECT ON public.mv_player_season_averages TO authenticated, anon, service_role;
...
REVOKE ALL ON FUNCTION public.refresh_player_search_caches() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.refresh_player_search_caches() FROM anon;
REVOKE ALL ON FUNCTION public.refresh_player_search_caches() FROM authenticated;
GRANT EXECUTE ON FUNCTION public.refresh_player_search_caches() TO service_role;

DO $cron$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    PERFORM cron.unschedule('refresh-player-season-averages') WHERE EXISTS (
      SELECT 1 FROM cron.job WHERE jobname = 'refresh-player-season-averages'
    );
    PERFORM cron.unschedule('refresh-player-search-caches') WHERE EXISTS (
      SELECT 1 FROM cron.job WHERE jobname = 'refresh-player-search-caches'
    );
    PERFORM cron.schedule(
      'refresh-player-search-caches',
      '40 12 * * *',
...
GRANT EXECUTE ON FUNCTION public.search_players(text, text, text[], uuid, text[], text[], uuid[], uuid[], boolean, text, text, text, int, int, int) TO authenticated, service_role;
