-- Make auto_set_lineup_atomic enforce the same authoritative lineup invariants
-- as set_player_slot_atomic. This RPC is granted to authenticated users and
-- replaces an entire day, so it must not trust the client to preserve locked
-- entries or submit only legal starter slots.
...
GRANT EXECUTE ON FUNCTION public.auto_set_lineup_atomic(uuid, uuid, uuid, date, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.auto_set_lineup_atomic(uuid, uuid, uuid, date, jsonb) TO service_role;
