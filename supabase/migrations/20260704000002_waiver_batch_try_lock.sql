-- Waiver cron: stop serializing on a blocking league advisory lock.
-- process_next_waiver_claim_atomic now walks the due-claim candidates and
-- pg_try_advisory_xact_lock's each league, skipping contended leagues for the
-- current pass -- the same try-lock-and-skip pattern the trade and nomination
-- batch processors use. Within the batch transaction the acquired lock is
-- retained, so per-league claim ordering is unchanged.
